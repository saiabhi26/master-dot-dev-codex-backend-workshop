...
test('Bidding War accepts only valid concurrent Bids in four tiled windows', async ({ browser }) => {
  ...
  try {
    ...
    const closesAt = Date.now() + 120_000;
    ...
    await Promise.all(bidders.map((page, index) => labelWindow(page, `BIDDER ${index + 1} · ${bidderIdentities[index]}`, ['#dc2626', '#2563eb', '#7c3aed'][index])));
    const acceptedConcurrentBids = await bidders[0].locator('.bid-history li').count();
    expect(acceptedConcurrentBids).toBe(1);
    await expect(bidders[0].locator('.bid-history li').filter({ hasText: '$110' }))
      .toHaveCount(1);
    await expect(bidders[0].locator('.current-price')).toContainText('$110');
    await expect(bidders[0].locator('.bid-minimum')).toContainText('$111');
