...
type PaymentStatusUpdate = { auctionId: number; status: 'Paid' };
...
export function subscribeToAuction(
  ...
  applyPaymentStatus: (status: 'Paid') => void,
  ...
  socket.on('payment:updated', (update: PaymentStatusUpdate) => {
    if (update.auctionId === auctionId) applyPaymentStatus(update.status);
  });
