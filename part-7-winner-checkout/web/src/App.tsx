...
import { BidRuleError, type AuctionCategory, type AuctionItem, type AuctionNotification, type DemoIdentity, type WinnerCheckout, auctionCategories, beginWinnerCheckout, createAuction, demoIdentities, fetchAuction, fetchAuctions, fetchWinnerCheckout, formatCurrency, formatTimeLeft, markNotificationRead, placeBid } from './catalog';
...
function useAuction(id: number | null, refreshToken = 0): Loadable<AuctionItem> & {
  replaceData: (auction: AuctionItem) => void;
  replacePaymentStatus: (status: 'Pending' | 'Paid') => void;
} {
  const [state, setState] = useState<Loadable<AuctionItem>>({ data: null, error: null });
  useEffect(() => {
    if (id === null) {
  ...
  return {
    ...
    replaceData: (auction) => setState((current) => (
      current.data?.id === auction.id && current.data.bidCount > auction.bidCount
        ? current
        : { data: { ...auction, paymentStatus: auction.paymentStatus ?? current.data?.paymentStatus }, error: null }
    )),
    replacePaymentStatus: (status) => setState((current) => (
      current.data ? { data: { ...current.data, paymentStatus: status }, error: null } : current
    )),
  };
}
...
function formatPaymentTime(paidAt: string) {
  return new Intl.DateTimeFormat(undefined, { dateStyle: 'long', timeStyle: 'short' }).format(new Date(paidAt));
}

function CheckoutPage({ id, identity }: { id: number | null; identity: DemoIdentity | null }) {
  const [checkout, setCheckout] = useState<WinnerCheckout | null>(null);
  const [error, setError] = useState<string | null>(null);
  ...
  useEffect(() => {
    document.title = 'Winner Checkout · Auction House';
    if (id === null || !identity) {
      setCheckout(null);
      setError(id === null ? 'Auction not found.' : null);
      setLoading(false);
      return;
    }
    const controller = new AbortController();
    setLoading(true);
    setError(null);
    fetchWinnerCheckout(id, identity, controller.signal)
      .then(setCheckout)
      .catch((checkoutError: unknown) => {
        if (!(checkoutError instanceof DOMException && checkoutError.name === 'AbortError')) {
          setError(checkoutError instanceof Error ? checkoutError.message : 'Winner Checkout could not be loaded.');
        }
      })
  ...
  async function continueToPayment() {
    if (id === null || !identity) return;
    setSubmitting(true);
    setError(null);
    try {
      const result = await beginWinnerCheckout(id, identity);
      setCheckout(result.checkout);
      if (result.checkout.status === 'paid') return;
      if (!result.checkoutUrl) throw new Error('Hosted checkout is unavailable. Try again.');
      window.location.assign(result.checkoutUrl);
    } catch (checkoutError) {
      setError(checkoutError instanceof Error ? checkoutError.message : 'Winner Checkout could not be prepared.');
      setSubmitting(false);
    }
  }

  if (!identity) return <main className="checkout-page container"><div className="breadcrumbs"><a href="/">Home</a><span>/</span><span>Winner Checkout</span></div><section className="checkout-message"><p className="eyebrow">Winner Checkout</p><h1>Choose your demo user.</h1><p>Select the winning Bidder in the header to access this payment.</p></section></main>;
  if (loading) return <main className="checkout-page container"><section className="checkout-message"><p className="eyebrow">Winner Checkout</p><h1>Loading your checkout…</h1><p>Confirming the Auction Result and latest payment status.</p></section></main>;
  if (error || !checkout) return <main className="checkout-page container"><div className="breadcrumbs"><a href="/">Home</a><span>/</span><span>Winner Checkout</span></div><section className="checkout-message checkout-error"><p className="eyebrow">Winner Checkout</p><h1>Checkout unavailable</h1><p role="alert">{error ?? 'Winner Checkout could not be loaded.'}</p>{id !== null && <a href={`/auctions/${id}`}>Return to Auction</a>}</section></main>;

  const paid = checkout.status === 'paid';
  return <main className="checkout-page container"><div className="breadcrumbs"><a href="/">Home</a><span>/</span><a href={`/auctions/${checkout.auctionId}`}>{checkout.auctionTitle}</a><span>/</span><span>Winner Checkout</span></div><div className="checkout-layout"><section className="checkout-card"><p className="eyebrow">{paid ? 'Payment receipt' : 'You won the Auction'}</p><h1>{paid ? 'Payment complete.' : 'Complete your Winner Checkout.'}</h1>{outcome === 'canceled' && !paid && <p className="checkout-notice" role="status">Payment was canceled. Your Winner Payment is still Pending.</p>}<div className="checkout-line-item"><div><strong>{checkout.auctionTitle}</strong><span>Winning Bid · USD</span></div><b>{formatCurrency(checkout.finalPriceCents)}</b></div><dl className="checkout-facts"><div><dt>Winning Bidder</dt><dd>{checkout.winner}</dd></div><div><dt>Seller</dt><dd>{checkout.seller}</dd></div><div><dt>Status</dt><dd><span className={`payment-status ${paid ? 'paid' : 'pending'}`}>{paid ? 'Paid' : 'Pending'}</span></dd></div>{checkout.paidAt && <div><dt>Paid</dt><dd>{formatPaymentTime(checkout.paidAt)}</dd></div>}{checkout.stripeSessionId && <div><dt>Payment reference</dt><dd className="payment-reference">{checkout.stripeSessionId}</dd></div>}</dl>{paid ? <div className="checkout-complete" role="status"><span>✓</span><div><strong>Paid in full</strong><p>Auction House recorded the complete payment for this Auction Result.</p></div></div> : <><button className="checkout-action" disabled={submitting} onClick={() => void continueToPayment()}>{submitting ? 'Preparing secure checkout…' : 'Continue to payment'}</button><p className="checkout-secure">You’ll enter test card details on the hosted mock Stripe checkout. Auction House never stores them.</p></>}</section><aside className="checkout-aside"><span className="avatar">{identity.charAt(0).toUpperCase()}</span><div><small>Checking out as</small><strong>{identity}</strong><p>Only the winning Bidder can access and pay this Auction Result.</p></div></aside></div></main>;
}
...
function ItemPage({ id, identity }: { id: number | null; identity: DemoIdentity | null }) {
  ...
  const { data: item, error, replaceData, replacePaymentStatus } = useAuction(id, refreshToken);
  useEffect(() => {
    if (id === null) {
    ...
    return subscribeToAuction(id, replaceData, replacePaymentStatus, setRealtimeStatus, setRealtimeInstanceId);
  }, [id]);
  useEffect(() => {
    if (!item) return;
...
export function App() {
  ...
  const [identity, setIdentity] = useState<DemoIdentity | null>(() => {
    const stored = window.sessionStorage.getItem('auction-house-demo-identity');
  ...
  useEffect(() => {
    if (identity) window.sessionStorage.setItem('auction-house-demo-identity', identity);
  }, [identity]);
  useEffect(() => {
    setNotifications([]);
  ...
  async function followNotification(notification: AuctionNotification) {
    const dismissed = await dismissNotification(notification);
    if (!dismissed) return;
    window.location.assign(notification.kind === 'winner' ? `/auctions/${notification.auctionId}/checkout` : `/auctions/${notification.auctionId}`);
  }
  useEffect(() => {
