...
type PaymentStatusUpdate = import('./app.js').PaymentStatusUpdate;

const runtimeSchema = z.object({
  API_PORT: z.coerce.number().int().min(1).max(65_535).default(3107),
...
type ServerToClientEvents = {
  ...
  'payment:updated': (update: PaymentStatusUpdate) => void;
...
const app = buildApp({
  ...
  onPaymentUpdated(update) {
    io.to(`auction:${update.auctionId}`).emit('payment:updated', update);
  },
