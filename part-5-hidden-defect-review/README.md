...
`npm start` ensures the shared PostgreSQL and Redis containers and the `auction_part_5`
database are ready, runs pending migrations, and starts both apps. Open
<http://localhost:5105> (API `3105`).
...
To run the deterministic distributed topology, where sockets connect to API A on 3105 and
REST traffic is proxied to API B on 3205:
